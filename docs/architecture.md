...
- **File attachments** — Attachments are uploaded via `multipart/form-data` to `POST /tasks/{id}/attachments`, stored on disk in an `uploads/` Docker volume, and served via `GET /tasks/{id}/attachments/{att_id}` with the correct MIME type. Download uses `fetch` + `blob` + `URL.createObjectURL` to pass auth headers cross-origin. Maximum file size is 10 MB, validated on the backend before write.
- **Delete from detail page** — A red trash button with `confirm()` dialog is placed in the task detail header next to the back button. On success, the user is redirected to `/tasks`. The existing `useDeleteTask` hook handles the mutation and toast notification.
